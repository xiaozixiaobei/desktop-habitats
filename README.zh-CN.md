...
鱼会对你的光标做出反应，也会为食物争抢；水草在缓慢的水流里摇摆。有两个环境：**Riverscape（河景缸）**，一个种满水草的淡水缸；**Reefscape（礁岩缸）**，一个海水缸。

![Reefscape，一个海水缸，小丑鱼围绕海葵](docs/images/reefscape-wide.png)

场景用 Three.js 和 WebGL2 实时渲染。一切都在本机运行，装好之后不需要账号、也不需要联网。桌面壁纸目前**只支持 macOS**；两个环境也都能在浏览器里跑。Mac 应用启动时是 Riverscape，并会记住你在菜单里选择的环境。
...
- **Environment（场景）**：在 Riverscape 和 Reefscape 之间切换所有屏幕，并记住你的选择。
- **Feed（喂食）**：给每个屏幕的缸里投入十粒鱼食，礁岩缸是八粒。没被吃掉的鱼食会在模拟时间 20–40 秒后溶解（河景缸），礁岩缸是 36 秒，从落水那一刻算起。
...
两个场景用同一套质量档位，并在暂停或隐藏时停止渲染。壁纸还会响应窗口遮挡、电池供电、低电量模式和屏幕休眠。
...
不会。壁纸不监听其他应用里的输入，也不记录按键。两个浏览器预览在水族箱获得焦点时支持空格键暂停/继续、F 键全屏、H 键隐藏或显示控制栏。
...
- 两个场景都有 **Pause / Resume（暂停/继续）**、**Feed（喂食）**、**Fullscreen（全屏）** 和 **Hide controls（隐藏控制栏）**。**Show controls** 把控制栏调回来。
- 按 **空格** 暂停或继续，**F** 全屏，**H** 在水族箱获得焦点时隐藏或显示控制栏。
- **Quality（画质）** 提供 Eco（20 fps）、Balanced（30 fps，默认）和 Detail（60 fps）。这个选择在两个场景之间共享并被记住。这些是帧率上限；更低的档位同时也会降低渲染分辨率。
