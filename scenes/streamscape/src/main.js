...
import { createComposite } from '../../riverscape/src/composite.js';
import * as THREE from "three";
import { createEnvironment, createParticles } from "./environment.js";
import { createSimulation } from "./simulation.js";
import { createFishView } from "./fish-model.js";
import { createFood } from "./food.js";
import { randomGenerator } from "../../shared/random.js";
import { waterTime } from "../../riverscape/src/water.js";
import { createFrameLoop } from "../../shared/frame-loop.js";
import { renderSettings, framebufferSize } from "../../riverscape/src/render-policy.js";
...
async function start() {
  ...
  scene.background = new THREE.Color("#0b1918");
  ...
  scene.fog = new THREE.FogExp2("#1a302b", 0.043);
  const camera = new THREE.PerspectiveCamera(25.8, 1420 / 740, 0.2, 65);
  camera.position.set(0, 5.0, 21.0);
  ...
  scene.add(new THREE.HemisphereLight(0xd1e8eb, 0x35352a, 0.28));
  const key = new THREE.DirectionalLight(0xfff2d9, 4.0);
  ...
  // The dark backboard catches the overhead light and the boulders' shadows,
  // so gaps between blades read as lit water in front of a wall rather than a void.
  const backboard = new THREE.Mesh(
    new THREE.PlaneGeometry(44, 24),
    new THREE.MeshStandardMaterial({ color: 0x132b23, roughness: 1 }),
  );
  ...
  const fish = createFishView(scene, simulation);
  const food = createFood(scene, simulation);
  ...
  // Mesh transforms are static. Fish/food use instance matrices; particles
  ...
  function renderFrame(dt, now) {
    ...
    for (let i = 0; total > 0 && i < steps; i++) {
      ...
      simulation.update(step, pointer);
    }
    fish.update();
    food.update();
  ...
  window.habitatStats = () => ({
    ...
    fish: simulation.fish.length, food: { ...simulation.stats, remaining: simulation.pellets.length }, loop: loop.state,
    pointer: { ...simulation.interaction, alarmed: simulation.fish.filter(f => f.alarmUntil > simulation.time).length,
      active: Boolean(pointer), speed: pointer?.velocity.length() || 0,
      position: query.get('diagnostics') === '1' ? pointer?.position.toArray() : undefined },
    fishPixels: query.get('diagnostics') === '1' ? simulation.fish.map(f => {
      const p = f.position.clone().project(camera);
      return [(p.x + 1) * canvas.clientWidth / 2, (1 - p.y) * canvas.clientHeight / 2,
        f.position.z, f.heading.x, f.heading.y, f.heading.z];
    }) : undefined,
  });
